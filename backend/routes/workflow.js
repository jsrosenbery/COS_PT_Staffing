...
async function getDivisionSectionIds(termCode, division) {
  const result = await query(
...
async function getProtectedWork(termCode, division) {
  const sectionIds = await getDivisionSectionIds(termCode, division);
...
async function appendDecisionLog({
  ...
  await query(
    ...
    [eventType, actorName, actorRole, division, term, sectionKey, instructorName, oldValue, newValue, detail, source]
  );
}
...
router.post("/upload/schedule", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await appendDecisionLog({
      ...
      term: termCode,
      note: `Uploaded ${inDivision.length} section row(s) for ${divisionName}.`,
      source: "schedule_upload",
    });
  ...
  try {
    ...
    const result = await query(
      `SELECT s.assignment_group_id, s.primary_subject_course, s.primary_crn, s.title, s.division, s.campus,
              ...
              pt.employee_id, CONCAT_WS(' ', pt.first_name, pt.last_name) AS faculty_name, pt.seniority_rank,
              p.preference_rank
       FROM scope_sections s
...
router.post("/assignments", async (req, res) => {
  ...
  try {
    ...
    const section = await client.query(
      `SELECT division, primary_subject_course FROM scope_sections WHERE term_code = $1 AND assignment_group_id = $2 LIMIT 1`,
      [termCode, assignmentGroupId]
    );
    await client.query(
      `INSERT INTO scope_audit_log
        (event_type, actor_name, actor_role, division, term, section_key, instructor_name, note, source)
       VALUES ('assignment_saved', $1, 'admin', $2, $3, $4, $5, $6, 'assignment')`,
      [
        actorName,
        section.rows[0]?.division || "",
        termCode,
        assignmentGroupId,
        facultyName,
        reason ? `Tentative assignment saved. ${reason}` : `Tentative assignment saved.`,
      ]
    );
...
router.delete("/assignments/:id", async (req, res) => {
  ...
  try {
    ...
    const section = await client.query(
      `SELECT division FROM scope_sections WHERE term_code = $1 AND assignment_group_id = $2 LIMIT 1`,
      [row.term_code, row.assignment_group_id]
    );
    await client.query(
      `INSERT INTO scope_audit_log
        (event_type, actor_name, actor_role, division, term, section_key, instructor_name, note, source)
       VALUES ('assignment_removed', $1, 'admin', $2, $3, $4, $5, 'Tentative assignment removed.', 'assignment')`,
      [actorName, section.rows[0]?.division || "", row.term_code, row.assignment_group_id, row.faculty_name || row.employee_id]
    );
    await client.query("COMMIT");
...
router.put("/assignments/:id/reassign", async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO scope_audit_log
        (event_type, actor_name, actor_role, division, term, section_key, instructor_name, note, source)
       VALUES ('assignment_reassigned', $1, 'admin', $2, $3, $4, $5, $6, 'assignment')`,
      [actorName, section.rows[0]?.division || "", row.term_code, row.assignment_group_id, facultyName, reason]
    );
    await client.query("COMMIT");
...
router.post("/preferences", async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO scope_audit_log
        (event_type, actor_name, actor_role, term, instructor_name, note, source)
       VALUES ('preferences_saved', $1, 'faculty', $2, $3, $4, 'preferences')`,
      [facultyName || employeeId || facultyId, termCode, facultyName || employeeId || facultyId, `Saved ${preferences.length} preference(s).`]
    );
    await client.query("COMMIT");
