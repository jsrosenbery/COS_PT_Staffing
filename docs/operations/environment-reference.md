...
| `DATABASE_SSL_REJECT_UNAUTHORIZED` | Optional TLS verification flag | Defaults to `true`, requiring a certificate trusted by the runtime. Configure the provider CA when necessary. | `false` disables server-certificate authentication. Use it only as a documented, temporary compatibility exception on a separately authenticated private network. |
